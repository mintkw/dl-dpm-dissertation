...
if __name__ == "__main__":
    ...
    # Note that due to the naming convention which appends noise * 100 to the filename,
    # precision greater than 2 decimal places is not recommended.
    noise_level_all = [(10 * i + 5) / 100 for i in range(6)]  # 0.05, 0.15, ..., 0.55
    ...
    with open('robustness_to_noise.csv', 'w', newline='') as output_f:
        ...
        for i in range(len(noise_level_all)):
            n_biomarkers = 10
            noise_level = noise_level_all[i]
            sds_normal = np.ones(n_biomarkers) * noise_level
            sds_abnormal = np.ones(n_biomarkers) * noise_level
            ...
            # Generate datasets.
            file_name = f"synthetic_{12 * n_biomarkers}_{n_biomarkers}_{int(noise_level * 100)}"
            generate_data(n_biomarkers=n_biomarkers, n_mci=100 * n_biomarkers,
                          n_controls=10 * n_biomarkers, n_patients=10 * n_biomarkers,
                          num_sets=n_trials, sds_normal=sds_normal, sds_abnormal=sds_abnormal,
                          file_name=file_name)
            ...
            for trial in range(n_trials):
                ...
                print(f"Now training on dataset with {noise_level} stdev  ({trial + 1}/{n_trials} trials)")
    ...
    ax.plot(np.array(noise_level_all), np.array(time_taken_vae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(time_taken_vae).mean(axis=-1),
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_level_all), np.array(time_taken_ae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(time_taken_ae).mean(axis=-1),
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_level_all), np.array(sequence_scores_vae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(sequence_scores_vae).mean(axis=-1),
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_level_all), np.array(sequence_scores_ae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(sequence_scores_ae).mean(axis=-1),
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_level_all), np.array(staging_errors_vae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(staging_errors_vae).mean(axis=-1),
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_level_all), np.array(staging_errors_ae).mean(axis=-1))
    ax.errorbar(np.array(noise_level_all), np.array(staging_errors_ae).mean(axis=-1),
