...
if __name__ == "__main__":
    ...
    with open('scalability_with_dimensionality.csv', 'w', newline='') as output_f:
        ...
        writer.writerow(['mean_ae_time', 'ae_time_std', 'mean_vae_time', 'vae_time_std',
                         'mean_ae_staging', 'ae_staging_std', 'mean_vae_staging', 'vae_staging_std',
                         'mean_ae_kt', 'ae_kt_std', 'mean_vae_kt', 'vae_kt_std',
                         'mean_ae_reconst', 'ae_reconst_std', 'mean_vae_reconst', 'vae_reconst_std'])
        ...
        for i in range(len(n_biomarkers_all)):
            ...
            for trial in range(n_trials):
                ...
                staging_rmse, reconstruction_error = evaluate_autoencoder(test_loader, ae, DEVICE)
                ...
                reconstruction_errors_ae[-1][trial] = reconstruction_error.cpu()
                ...
                reconstruction_errors_vae[-1][trial] = reconstruction_error.cpu()

            # ----------- Write results to file -----------
            writer.writerow([time_taken_ae[-1].mean().item(),
                             time_taken_ae[-1].std().item(),
                             time_taken_vae[-1].mean().item(),
    ...
    print(time_taken_ae)
    print(time_taken_vae)
    print(staging_errors_ae)
    print(staging_errors_vae)
    print(sequence_scores_ae)
    print(sequence_scores_vae)
    print(reconstruction_errors_ae)
    print(reconstruction_errors_vae)
