...
if __name__ == "__main__":
    ...
    pretrained_model_name = f"pretrained_82bm"  # run pretrain_a_model with the expected number of biomarkers first.
    n_trials = 1  # Set the number of times to repeat the experiment.

    dataset_name = "adni_all_volumes_abpos"
    ...
    for trial in range(n_trials):
        ...
        for i in range(len(model_types)):
            ...
            # Get predictions on and diagnosis labels from the dataset
            preds = []
            diagnoses = []
            with torch.no_grad():
            ...
            # Predict sequence
            sequence_prediction = autoencoder_sequence_inference.infer_seq_from_network(dataloader=data_loader,
                                                                                        net=net).cpu()
            ...
            if trial == n_trials - 1:
                ...
                # fig, ax = plotting.plot_predicted_sequence(gt_ordering=sequence_prediction, pred_ordering=sequence_prediction,
                #                                            biomarker_names=biomarker_names)
                # fig.suptitle(f"Visualisation of {model_type.upper()} ordering")
                # fig.show()

                # PVD (with y axis sorted by the seq computed using the full dataset)
                fig, ax = plotting.positional_var_diagram(gt_ordering=sequence_prediction, dataset=train_dataset, net=net,
                                                          biomarker_names=biomarker_names, n_samples=100)
    ...
    for i in range(len(model_types)):
        print(f"Reconstruction losses of {model_types[i]}:", reconstruction_errors[i])
        print(
            f"Reconstruction MSE of {model_types[i]}: {reconstruction_errors[i].mean()} +- {reconstruction_errors[i].std()}")
        print()
