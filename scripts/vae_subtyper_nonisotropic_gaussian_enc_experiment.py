...
class Encoder(nn.Module):
    ...
    def __init__(self, n_biomarkers, d_latent, n_subtypes):
        ...
        self.fc_stage_sigma = nn.Linear(16, self.d_latent)
        self.fc_subtype_sigma = nn.Linear(16, self.n_subtypes * self.n_subtypes)
    ...
    def forward(self, X):
        ...
        stage_log_sigma = self.fc_stage_sigma(h)
        stage_sigma = torch.exp(stage_log_sigma)  # exponentiate to enforce non-negativity

        subtype_log_sigma = self.fc_subtype_sigma(h)
        subtype_log_sigma = torch.reshape(subtype_log_sigma, (-1, self.n_subtypes, self.n_subtypes))

        # Concatenate the computed stage and subtype means
        mu = torch.concatenate([stage_mu, subtype_mu], dim=-1)
        sigma = torch.concatenate([torch.zeros((X.shape[0], self.n_subtypes, 1), device=DEVICE),
                                  torch.matrix_exp(subtype_log_sigma)], dim=-1)
        # todo: the below assumes sigma is a scalar but it isn't - it's a vector. deal w it accordingly
        sigma = torch.concatenate([torch.concatenate([torch.ones(1, device=DEVICE) * stage_sigma,
                                  torch.zeros(self.n_subtypes, device=DEVICE)]).repeat((X.shape[0], 1, 1)),
                                   sigma], dim=1)
        ...
        return dist.MultivariateNormal(mu, sigma), subtype_log_sigma
...
def compute_elbo(vae, X, device, beta):
    ...
    subtype_prior_cov = torch.tensor([[1., -0.5], [-0.5, 1.]], device=DEVICE)
    subtype_prior = dist.MultivariateNormal(torch.ones(n_subtypes, device=DEVICE), subtype_prior_cov)
    ...
    kl_div = q_z.log_prob(z).sum(dim=-1) - pseudostage_prior.log_prob(z[:, 0]) - subtype_prior.log_prob(z[:, 1:])
