# A script to generate fixed-size datasets with different noise levels, fit models to them,
# and plot evaluation metrics against noise level.
...
if __name__ == "__main__":
    ...
    noise_levels = [i/100 for i in range(5, 60, 10)]

    # Define dataset names
    n_biomarkers = 10
    # dataset_names = [f"synthetic_{12 * n_biomarkers}_{n_biomarkers}_0" for n_biomarkers in noise_levels]
    ...
    with open('../data_noise_comparison_results.csv', 'w', newline='') as output_f:
        writer = csv.writer(output_f)
        writer.writerow(['vae_reconstruction', 'ae_reconstruction', 'vae_staging', 'ae_staging'])
        ...
        for noise_level in noise_levels:
            ...
            for trial in range(n_trials):
                ...
                print(f"Now training on dataset with {noise_level} std noise in the data ({trial+1}/{n_trials} trials)")
    ...
    ax.plot(np.array(noise_levels), np.array(reconstruction_errors_vae))
    fig.suptitle("Mean squared reconstruction error of VAE")
    ax.set_xlabel("STD of Gaussian noise in the data")
    ax.set_ylabel("Reconstruction error")
    fig.show()

    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_levels), np.array(reconstruction_errors_ae))
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_levels), np.array(staging_errors_vae))
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(np.array(noise_levels), np.array(staging_errors_ae))
